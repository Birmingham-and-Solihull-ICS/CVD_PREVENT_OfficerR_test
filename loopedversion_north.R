...
dt_PCN_ethn_north <- dbGetQuery(con,
                     "Select a.*, b.Abbreviated, c.Locality
                      from EAT_Reporting_BSOL.[Development].[BSOL_1255_CVDP_Data] a inner join
                      EAT_Reporting_BSOL.[Development].[BSOL_1255_CVDP_Data_PCNs] b on a.AreaCode = b.AreaCode
...
for(i in inds){
  ...
  c<-
    ...
    arrange(Value) %>%
    mutate(AreaName=factor(AreaName, levels=AreaName),
           `North Locality` = ifelse(Locality == "North", TRUE,FALSE)) %>%
    ggplot()+
    geom_col(aes(x=AreaName, y= Value, fill = `North Locality`), position = position_identity())+
  ...
  #unique(dt_PCN_ethn_north$AreaName)
  ...
  d<-
    ...
    mutate(PCN_mask = case_when(
      AreaName == "North Birmingham PCN" ~ "A"
      , AreaName == "SUTTON GROUP PRACTICE PCN" ~ "B"
      , AreaName == "MMP CENTRAL AND NORTH PCN" ~ "C"
      , AreaName == "Gosk PCN" ~ "D"
      , AreaName == "Kingstanding, Erdington & Nechells PCN" ~ "E"
      , AreaName == "Alliance Of Sutton Practices PCN" ~ "F"
...
##############################################
 ...
 # PCN
  ...
  # Add title
  my_pres2 <- ph_with(my_pres2, value = paste("North Locality PCNs: Ethnicity -", sh_title), location =  ph_location_label("Slide Title"))
...
print(my_pres2, target = "output/north_example.pptx")
